...
const SECTIONS: { key: keyof Omit<CompatibilityData, "score">; label: string; icon: string }[] = [
  { key: "harmony", label: "Overall Harmony", icon: "\uD83D\uDC91" },
  { key: "communication", label: "Communication", icon: "\uD83D\uDCAC" },
  { key: "emotional", label: "Emotional Connection", icon: "\uD83D\uDC96" },
  { key: "chemistry", label: "Romantic Chemistry", icon: "\uD83D\uDD25" },
  { key: "challenges", label: "Challenges", icon: "\u26A0\uFE0F" },
  { key: "advice", label: "Relationship Advice", icon: "\uD83D\uDCA1" },
];

function getCacheKey(chartA: NatalChart, form: PartnerForm): string {
  const a = JSON.stringify(chartA.planets.slice(0, 3).map((p) => p.longitude));
  const b = `${form.year}-${form.month}-${form.day}-${form.hour}-${form.minute}`;
  return `astro_compat_${a}_${b}`;
}
...
function getCached(key: string): CompatibilityData | null {
  ...
  try {
    ...
    if (!cachedAt || Date.now() - cachedAt > COMPAT_CACHE_TTL_MS) {
...
function cacheResult(key: string, data: CompatibilityData) {
...
export default function AstroCompatibility({ chart }: Props) {
  ...
  async function handleCheck() {
    ...
    try {
      const birthInfoB: AstroBirthInfo = {
        year: parseInt(partner.year),
        month: parseInt(partner.month),
        day: parseInt(partner.day),
        hour: parseInt(partner.hour) || 12,
        minute: parseInt(partner.minute) || 0,
        latitude: parseFloat(partner.latitude) || 40.7128,
        longitude: parseFloat(partner.longitude) || -74.006,
        cityName: partner.cityName || "New York",
      };
  ...
  return (
    <div className="rounded-2xl border border-gold/20 bg-background p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gold">Astro Compatibility</h3>
      ...
      {!result && (
        <div className="space-y-4">
          <p className="text-sm text-foreground/50">
            Enter the other person&apos;s birth information to check your astrological compatibility.
          </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-foreground/40 mb-1">Hour (0-23)</label>
              <input
                type="number"
                value={partner.hour}
                onChange={(e) => setPartner({ ...partner, hour: e.target.value })}
                placeholder="12"
                min={0}
                max={23}
                className="w-full rounded-lg border border-border bg-white/5 px-3 py-2 text-sm text-foreground placeholder:text-foreground/20 focus:border-gold/40 focus:outline-none"
            ...
            <div>
              <label className="block text-xs text-foreground/40 mb-1">Minute (0-59)</label>
              <input
                type="number"
                value={partner.minute}
                onChange={(e) => setPartner({ ...partner, minute: e.target.value })}
                placeholder="0"
                min={0}
                max={59}
                className="w-full rounded-lg border border-border bg-white/5 px-3 py-2 text-sm text-foreground placeholder:text-foreground/20 focus:border-gold/40 focus:outline-none"
          ...
          <div>
            <label className="block text-xs text-foreground/40 mb-1">City (optional)</label>
            <input
              type="text"
              value={partner.cityName}
              onChange={(e) => setPartner({ ...partner, cityName: e.target.value })}
            ...
            {SECTIONS.map(({ key, label, icon }) => {
              ...
              return (
                <div key={key} className="rounded-xl border border-border/20 bg-white/3 p-4">
                  ...
                  <p className="text-sm text-foreground/60 leading-relaxed">{text as string}</p>
                </div>
              );
            })}
          </div>
