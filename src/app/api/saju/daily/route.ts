...
export async function POST(req: NextRequest) {
  try {
    ...
    const { energy, career, relationships, tip, favorableTime } = parsed;
    return NextResponse.json({ energy, career, relationships, tip, favorableTime });
  } catch (error) {
    console.error('Daily reading error:', error instanceof Error ? error.message : 'Unknown error');
